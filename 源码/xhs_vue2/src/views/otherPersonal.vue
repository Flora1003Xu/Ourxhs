...
export default {
  ...
  created(){
    ...
    this.$axios({
      method: 'get',
      url: 'http://localhost:8081/'+this.userId+'/'+this.watchId+'/PersonalView',
      headers: {
        'Content-Type': false,
  ...
  methods: {
    isfollowshow(){
      ...
      if(this.isfollow)
        {
          this.followButtonText = "关注"
          this.$axios.delete('http://localhost:8081/'+this.watchId+'/PersonalView/follow', {
        ...
        {
          this.followButtonText = "已关注"
          this.$axios.post('http://localhost:8081/'+this.watchId+'/PersonalView/follow', {
