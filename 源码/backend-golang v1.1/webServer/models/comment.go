...
// 评论信息
type Comment struct {
	CommentatorID   int64    `json:"commentatorId" form:"commentatorId"`     // 评论者账号
	NoteID          int      `json:"noteId" form:"noteId"`                   //评论所在的笔记编号
	CommentatorName string   `json:"commentatorName" form:"commentatorName"` // 评论者
	CommentID       int64    `json:"commentId" form:"commentId"`             // 评论编号
	CommentTime     string   `json:"commentTime" form:"commentTime"`         // 评论时间
	Content         string   `json:"content" form:"content"`                 // 评论内容
	Portrait        string   `json:"portrait" form:"portrait"`               // 用户头像
	State           int      `json:"state" form:"state"`                     //是否已读
	AtUserID        []int    `json:"atUserId" form:"atUserId"`               //@人的账号
	AtName          []string `json:"atName" form:"atName"`                   //@人的名字
	AtLocation      []int    `json:"atLocation" form:"atLocation"`           //@的位置
}
...
func GetCommentInfo(Id, option int) (comments []Comment, totalState int, ok bool) {
	...
	WHERE n.creatorAccount=? AND n.noteId=c.noteID AND c.commentatorId=u.userAccount
	ORDER BY commentTime DESC`
	if option == 0 {
		sqlstr = notesql
	} else {
...
func NewComment(nc Comment, noteId int) (int, bool) {
	...
	if err != nil {
		fmt.Printf("评论insert failed, err:%v\n", err)
		return -1, false
	}
	// 新插入数据的id
