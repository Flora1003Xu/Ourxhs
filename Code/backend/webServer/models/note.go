...
type DetailNote struct {
	...
	IsCollected bool `json:"isCollected" form:"isCollected"` // 是否收藏该篇
	IsFollowed  bool `json:"isFollowed" form:"isFollowed"`   // 是否关注该作者
	IsLiked     bool `json:"isLiked" form:"isLiked"`         // 是否点赞该篇
	CollectNum  int  `json:"collectNum" form:"collectNum"`   // 收藏数
}
...
func NewNoteInfo(nn DetailNote) (int, bool) {
	...
	(creatorAccount, cover, title, body, createTime, updateTime, location)
	VALUES
	(?,?,?,?,?,?,?)`
	ret, err := db.Exec(sqlstr, nn.CreatorID, nn.Cover, nn.Title, nn.Body, nn.CreateTime, nn.UpdateTime, nn.Location)
	if err != nil {
...
func SpecificNote(noteid int) (fullNote, bool) {
	var N fullNote
	//先找笔记信息
	sqlStr1 := `SELECT Noteid,CreatorAccount,Title,Body,NumOfPic,Cover,CreateTime,UpdateTime,Location,likeNum
	FROM noteInfo 
	WHERE noteId = ?`
	...
	for rows.Next() {
		...
		err := rows.Scan(&N.NoteInfo.NoteID, &N.NoteInfo.CreatorID, &N.NoteInfo.Title, &N.NoteInfo.Body, &N.NoteInfo.Picnum, &N.NoteInfo.Cover, &createTimestring, &updateTimestring, &N.NoteInfo.Location, &N.NoteInfo.LikedNum)
		if err != nil {
