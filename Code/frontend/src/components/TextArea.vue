  ...
  <div class="others">
    <el-popover placement="top-start" width="200" trigger="click" class="othersBox">
      <div class = "othersList">
          <div href="javascript:void(0);" @click="getothers(index)" v-for="(item,index) in others" :key= "index" class="othersone">
            <el-row type="flex" align="middle" >
              <el-avatar :size="40" :src="require('../assets/'+item.portrait)" ></el-avatar>&nbsp;&nbsp;{{item.userName}}
            </el-row>
          </div>
      </div>
      <el-button
      ...
      { //删除时，@整体删除
        ...
        for(let j in this.aiteList)
        {
          ...
          if(this.textarea[loc1] =="@")
          {
             ...
             {
              this.textarea=this.textarea.substring(0,loc2-1)+lastword+" ";
             }
          }
        }
        }
