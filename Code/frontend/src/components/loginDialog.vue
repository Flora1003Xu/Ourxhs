...
<template>
  ...
  >
    <el-form ref="form" label-position="left" label-width="80px">
      ...
      <el-form-item>
        <el-button type="primary" @click="$emit('input', false), submitLogin()"
          >登 录</el-button
        >
        <el-button @click="(regVisible = true), clearForm()"
          >新用户注册</el-button
        >
        ...
        >
          ...
          >
            ...
            <el-form-item>
              <el-button type="primary" @click="submitReg('form')"
                >注 册</el-button
              >
            </el-form-item>
          </el-form>
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        header: [{ required: true, message: "请选择头像" }],
      },
      imageUrl: "",
      token: "",
      err: "",
    ...
    submitLogin() {
      try {
        axios
          ...
          .then((response) => {
            if (response.data.code == 200) this.$message.success("登陆成功！");
            this.token = response.data.data;
            console.log("1231", this.token);
            GlobalToken.Token = this.token;
            if (this.token == "") this.$message.error("登陆失败！");
