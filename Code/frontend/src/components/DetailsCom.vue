        ...
        <el-container>
          ...
          >
            ...
            <el-row>
              ...
              <div
                ...
                :key="index"
                style="display: flex; flex-wrap: wrap; font-size: 18px"
              >
                <el-link
                  style="font-size: 18px"
                  :underline="false"
                  @click="To_personalView(index)"
                  >@{{ item }}</el-link
                >
            ...
            <el-row>
              <div class="timeclass">
                共{{ comments ? comments.length : 0 }}条评论
              </div> </el-row
            ><br />
            <!-- 一条条评论 -->
          ...
          >
            <el-row type="flex" align="middle">
              <el-col :span="1">
                <el-button
                  :icon="islike ? 'el-icon-solidheart' : 'el-icon-heart'"
                  type="text"
                  size="mini"
                  @click="changelike"
                ></el-button>
            ...
            <el-row type="flex" align="middle">
              ...
              <el-col :span="4">
                <el-button
                  style="display: block; margin: 0 auto; align: middle"
                  round
                  @click="commentsend"
                  >发送</el-button
                >
...
export default {
  ...
  methods: {
      ...
      if (event.which === 8) {
        ...
        for (let j in this.commentaiteList) {
          ...
          if (this.mycomment[loc1] == "@") {
            loc1++;
            while (
              this.mycomment[loc1] == this.commentaiteList[j][ailoc] &&
              ailoc < this.commentaiteList[j].length
            ) {
              loc1++;
              ailoc++;
            }
            if (ailoc == this.commentaiteList[j].length) {
              this.mycomment =
                this.mycomment.substring(0, loc2 - 1) + lastword + " ";
            }
          }
        }
      }
    ...
    async load_data(value) {
      ...
      this.imgList = [];
      await this.$axios
        .get("http://localhost:8081/" + this.myid + "/explore/" + this.noteID)
        .then((response) => {
      ...
      this.$axios
        .get("http://localhost:8081/comment/" + this.noteID)
        .then((response) => {
    ...
    commentchangelike(item) {
      ...
      if (item.cislike) item.clikenum += 1;
      else item.clikenum -= 1;
    },
    changecollect() {
      if (GlobalToken.Token == "") {
        //没权限
